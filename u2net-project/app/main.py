...
async def download_proxy(url: str):
    ...
    async def stream_generator():
        try:
            # Use client.stream() instead of client.get(..., stream=True)
            async with client.stream("GET", url, headers=headers) as response:
                if response.status_code not in (200, 206):
                    raise HTTPException(status_code=400, detail="Failed to fetch video from source.")
...
@app.get("/youtube-download/audio")
def download_audio_original(
    url: str = Query(...),
    itag: int = Query(...),
    background_tasks: BackgroundTasks = None
):
    try:
        ...
        # Get the audio stream by itag
        audio_stream = yt.streams.get_by_itag(itag)
        if not audio_stream or "audio" not in audio_stream.mime_type:
            return JSONResponse(status_code=400, content={"error": "Invalid or non-audio itag"})
        ...
        filename = f"{safe_title}_{itag}_{uid}.{ext}"
        ...
        return FileResponse(
            ...
            media_type=audio_stream.mime_type,
            background=background_tasks
        )

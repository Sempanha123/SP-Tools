...
const navSearchQuery = ref(typeof route.query.q === 'string' ? route.query.q : '')
const isCategoryActive = (slug: string) => route.path === `/news/category/${slug}`
const submitSearch = async () => { const query = navSearchQuery.value.trim(); await router.push({ path: '/news/search', query: query ? { q: query } : {} }) }
...
const categoryShortNameMap: Record<string,string> = { world:'World',asia:'Asia',americas:'Americas',europe:'Europe','middle east':'M. East',business:'Business',technology:'Tech',science:'Science',climate:'Climate',health:'Health' }
const getCategoryShortName = (name: string | null | undefined) => { const cleanName = name?.trim() ?? ''; if (!cleanName) return 'News'; return categoryShortNameMap[cleanName.toLowerCase()] ?? cleanName }
</script>

<template>
  <section class="sticky top-16 z-30 border-b border-line bg-surface/[0.90] shadow-xs backdrop-blur-2xl">
    <div class="sp-container-wide flex flex-col gap-2.5 py-2.5 xl:flex-row xl:items-center">
...
<style scoped>
.category-scroll{scrollbar-width:none}.category-scroll::-webkit-scrollbar{display:none}
</style>

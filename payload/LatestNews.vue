...
const filteredArticles = computed(() => {
  ...
  if (selectedRegion.value !== 'all') {
    ...
    result = result.filter(
      article => article.region.trim().toLowerCase() === selected,
    )
  }
  ...
  <section class="min-w-0">
    <div class="sp-reveal flex flex-col justify-between gap-5 border-b border-line pb-5 sm:flex-row sm:items-end">
      <div>
        ...
        <h2 class="mt-3 text-[clamp(2.1rem,4vw,3.25rem)] font-[720] leading-[1] tracking-[-0.052em] text-fg">
          News as it happens
        </h2>
        <p class="mt-3 max-w-xl text-[13px] leading-6 text-fg-muted">
          A faster scan of new reporting, followed by a denser three-column news wall.
        </p>
      </div>
    ...
    <div
      ...
      class="sp-news-latest-grid sp-stagger mt-7 grid gap-4 md:grid-cols-2 xl:grid-cols-3"
    >
    ...
    <div
      v-else
      class="sp-reveal sp-lens-card relative mt-7 overflow-hidden rounded-[24px] px-6 py-14 text-center shadow-lift sm:py-[72px]"
    >
      ...
      <div class="relative mx-auto max-w-md">
        <span class="mx-auto flex h-14 w-14 items-center justify-center rounded-[17px] border border-line bg-surface-3 text-fg-subtle shadow-xs">
          ⌕
        </span>
        <p class="mt-5 text-[10px] font-bold uppercase tracking-[0.14em] text-accent">
          Desk is quiet
        </p>
        <h3 class="mt-2 text-[22px] font-[700] tracking-[-0.035em] text-fg">
          No stories in this view yet.
        </h3>
        <p class="mt-3 text-[13px] leading-6 text-fg-muted">
          Once the news API responds, new coverage will appear here automatically.
        </p>
        <button
          type="button"
          class="sp-btn sp-btn-secondary mt-6 h-10 px-4 text-xs"
          @click="clearFilters"
        >
          Show all stories
        </button>
    ...
    <div v-if="canLoadMore" class="mt-9 text-center">
      <button
        type="button"
        class="sp-btn sp-btn-secondary h-11 px-5 text-xs"
